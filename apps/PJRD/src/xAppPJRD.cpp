...
usage::jpeg-specific --------------------------------------------------------
 ...
 -qtl  QuantTabLayout     Select quantization table layout used during encoding:
                          [Default, Flat, SemiFlat] (default Default) [optional]
                          Default is based on RFC2435
 -ri   RestartInterval    Restart interval in number of MCUs [0=disabled, -1=MCU row] (optional, default 0) 

usage::optimization ---------------------------------------------------------
 -oqc  OptQuantCoeffs     Enable RDOQ (default 1) [optional]
 ...
 -lem  LambdaEstMode      Lambda estimation mode (optional, default ApproxExact)
                          [Exhaustive, ApproxExact, ApproxFast]
 -dcm  DistCalkMode       Distortion calculation mode (optional, default Exact)
                          [Exact = calculates full reconstruction, Approx = estimates SSD based on DCT coefficients]
...
void xAppJPEG::registerCmdParams()
{
  ...
  m_CfgParser.addCmdParm("ent", "EntropyType"    , "", "EntropyType"    ); 
  //optimization
  m_CfgParser.addCmdParm("oqc", "OptQuantCoeffs"   , "", "OptQuantCoeffs"   );
  m_CfgParser.addCmdParm("pzc", "ProcessZeroCoeffs", "", "ProcessZeroCoeffs");
  m_CfgParser.addCmdParm("oet", "OptEntropyTables" , "", "OptEntropyTables" );
  m_CfgParser.addCmdParm("npb", "NumOptPassesBlock", "", "NumOptPassesBlock");
  m_CfgParser.addCmdParm("npp", "NumOptPassesPic"  , "", "NumOptPassesPic"  );
  m_CfgParser.addCmdParm("lem", "LambdaEstMode"    , "", "LambdaEstMode"    );
  m_CfgParser.addCmdParm("dcm", "DistCalkMode"     , "", "DistCalkMode"     );
  //validation 
...
bool xAppJPEG::readConfiguration()
{
  ...
  if(m_CfgParser.findParam("PictureFormat"))
  {
    ...
    std::tie(m_PictureType, m_ChromaFormat, m_BitDepth) = xFmtScn::scanPixelFormat(PictureFormatS);
    if(m_PictureType != eImgTp::YCbCr   ) { m_ErrorLog += "!  Invalid or unsuported ImageType value derrived from PictureFormat\n"; AnyError = true; }
  ...
  //jpeg-specific -----------------------------------------------------------------------------------------------------
  m_Implementation  = m_CfgParser.cvtParam1stArg("Implementation"  , eImpl::Advanced, xStrToImpl);
  ...
  //optimization ------------------------------------------------------------------------------------------------------
  m_OptQuantCoeffs    = m_CfgParser.getParam1stArg("OptQuantCoeffs"   , 1);
  m_ProcessZeroCoeffs = m_CfgParser.getParam1stArg("ProcessZeroCoeffs", 0);
  m_OptEntropyTables  = m_CfgParser.getParam1stArg("OptEntropyTables" , 1);
  ...
  m_LambdaEstMode     = m_CfgParser.cvtParam1stArg("LambdaEstMode"    , eLmbd::ApproxExact, xStrToLmbd  );
  m_DistCalkMode      = m_CfgParser.cvtParam1stArg("DistCalkMode"     , eCalkMd::Exact    , xStrToCalkMd);
  ...
  m_RestartIntervalUsed = m_RestartInterval < 0 ? xCodecCommon::calcNumMCUsInWidth(m_PictureSize, m_ChromaFormat) : m_RestartInterval;
...
std::string xAppJPEG::formatConfiguration()
{
  ...
  Config += fmt::format("RestartInterval   = {}{}\n", m_RestartInterval, m_RestartInterval == NOT_VALID ? "  (auto - one MCU row)" : "");
  //optimization
  Config += fmt::format("OptQuantCoeffs    = {}\n", m_OptQuantCoeffs   );
  Config += fmt::format("ProcessZeroCoeffs = {}\n", m_ProcessZeroCoeffs);
  Config += fmt::format("OptEntropyTables  = {}\n", m_OptEntropyTables );
  Config += fmt::format("NumOptPassesBlock = {}\n", m_NumOptPassesBlock);
  Config += fmt::format("NumOptPassesPic   = {}\n", m_NumOptPassesPic  );  
  Config += fmt::format("LambdaEstMode     = {}\n", xLmbdToStr  (m_LambdaEstMode));
  Config += fmt::format("DistCalkMode      = {}\n", xCalkMdToStr(m_DistCalkMode ));
  //validation 
  ...
  Config += fmt::format("RestartIntervalUsed = {:d}\n", m_RestartIntervalUsed);  
...
void xAppJPEG::setupMultithreading()
{
...
void xAppJPEG::ceaseMultithreading()
{
...
std::string xAppJPEG::formatMultithreading()
{
  std::string Info = "";
...
eAppRes xAppJPEG::setupSeqAndBuffs()
{
  ...
  {
    if(!xFile::exists(m_InputFile)) { xErrMsg::printError(fmt::format("ERROR --> InputFile does not exist ({})", m_InputFile)); return eAppRes::Error; }
  }
  else
  {
    const std::string InputFile0 = fmt::format(fmt::runtime(m_InputFile), 0);
    const std::string InputFile1 = fmt::format(fmt::runtime(m_InputFile), 1);
    if(!xFile::exists(InputFile0) && !xFile::exists(InputFile1)) { xErrMsg::printError(fmt::format("ERROR --> InputFile does not exist ({}) [Checked {} {}]", m_InputFile, InputFile0, InputFile1)); return eAppRes::Error; }
  }
  ...
  case eFileFmt::PNG: m_SeqOrg = new xSeqPNG(m_PictureSize, std::numeric_limits<uint16>::max()); break;
  case eFileFmt::BMP: m_SeqOrg = new xSeqBMP(m_PictureSize, std::numeric_limits<uint16>::max()); break;
  default: xErrMsg::printError(fmt::format("ERROR --> unsupported FileFormat ({})", xFileFmt2Str(m_FileFormat))); return eAppRes::Error;
  }
  {
    xSeqPic::tResult Result = m_SeqOrg->openFile(m_InputFile, xSeq::eMode::Read);
    if(!Result) { xErrMsg::printError(fmt::format("ERROR --> InputFile opening failure ({}) {}", m_InputFile, Result.format())); return eAppRes::Error; }
  }
  ...
  int32 NumOfFrames = 0;
  {
    ...
    if(m_StartFrame >= NumOfFrames) { xErrMsg::printError(fmt::format("ERROR --> StartFrame >= DetectedFrames for ({})", m_InputFile)); return eAppRes::Error; }
  }
  ...
  if(FirstFrame != 0)
  {
    xSeqPic::tResult Result = m_SeqOrg->seekFrame(FirstFrame);
    if(!Result) { xErrMsg::printError(fmt::format("ERROR --> InputFile seeking failure ({}) {}", m_InputFile, Result.format())); return eAppRes::Error; }
  }
  ...
  if(m_WriteRecon) 
  {
    ...
    case eFileFmt::PNG: m_SeqRec = new xSeqPNG(m_PictureSize, std::numeric_limits<uint16>::max()); break;
    case eFileFmt::BMP: m_SeqRec = new xSeqBMP(m_PictureSize, std::numeric_limits<uint16>::max()); break;
    default: xErrMsg::printError(fmt::format("ERROR --> unsupported FileFormat ({})", xFileFmt2Str(m_FileFormat))); return eAppRes::Error;
    }

    xSeqPic::tResult Result = m_SeqRec->openFile(m_ReconFile, xSeq::eMode::Write);
    if(!Result) { xErrMsg::printError(fmt::format("ERROR --> ReconFile opening failure ({}) {}", m_ReconFile, Result.format())); return eAppRes::Error; }
  }
  ...
  if(m_WriteBit)
  {
    ...
    if(!OpenSucces) { xErrMsg::printError(fmt::format("ERROR --> OutputFile opening failure ({})", m_OutputFile)); return eAppRes::Error; }
  }
...
void xAppJPEG::createProcessors()
{
  ...
  case eImpl::Simple:
    ...
    m_EncoderSimple.create(m_ThreadPool);
    m_EncoderSimple.init(m_PictureSize, m_ChromaFormat, m_Quality, m_RestartIntervalUsed);
    m_EncoderSimple.setEmit(true, true, true);
    m_EncoderSimple.setGatherTimeStats(m_PrintDebug);
    if(m_Decode)
    {
      m_DecoderSimple.setVerboseLevel(m_VerboseLevel);
      m_DecoderSimple.create(m_ThreadPool);
      m_DecoderSimple.setGatherTimeStats(m_PrintDebug);
    }
    break;
  case eImpl::Advanced:
    m_EncoderRDOQ.setVerboseLevel(m_VerboseLevel);
    m_EncoderRDOQ.create(m_PictureSize, m_ChromaFormat, m_ThreadPool);
    m_EncoderRDOQ.setOptQuant  (m_OptQuantCoeffs, m_ProcessZeroCoeffs, m_LambdaEstMode);
    m_EncoderRDOQ.setOptEntropy(m_OptEntropyTables);
    m_EncoderRDOQ.setOptPass   (m_NumOptPassesBlock, m_NumOptPassesPic);
    m_EncoderRDOQ.setOptClcMode(m_DistCalkMode, eCalkMd::Exact);
  ...
  case eImpl::Turbo:
    m_EncoderTurbo.create(m_PictureSize, m_ChromaFormat);
    m_EncoderTurbo.init(m_Quality, true, true, false, 0);
    m_EncoderTurbo.setGatherTimeStats(m_PrintDebug);
    m_DecoderTurbo.create(m_PictureSize, m_ChromaFormat);
    if(m_Decode)
    {
      m_DecoderTurbo.init(true);
      m_DecoderTurbo.setGatherTimeStats(m_PrintDebug);
    }
    break;
...
eAppRes xAppJPEG::processAllFrames()
{
  ...
  {
    if(m_PrintDebug) { fmt::print("Frame {:08d} ", f); if(m_VerboseLevel > 9) { std::fflush(stdout); }}

    uint64 T0 = m_GatherTime ? xTSC() : 0;

    //reading
    xSeqPic::tResult ReadResult = !m_CvtClrSpc ? m_SeqOrg->readFrame(m_PicOrg4XX) : m_SeqOrg->readFrame(m_PicOrgRGB);
    if(!ReadResult) { xErrMsg::printError(fmt::format("ERROR --> InputFile read error ({}) {}", m_InputFile, ReadResult.format())); return eAppRes::Error; }
    ...
    switch(m_Implementation)
    {
      ...
      case eImpl::Advanced: m_EncoderRDOQ  .encode(m_PicOrg4XX, m_PicOrgRGB, &m_OutBuffer, true); break;
#if X_PMBB_HAS_JPEG_TURBO
      case eImpl::Turbo   : m_EncoderTurbo .encode(m_PicOrg4XX, &m_OutBuffer); break;
#endif //X_PMBB_HAS_JPEG_TURBO
      default: return eAppRes::Error; break;
    }    
    ...
    if(m_Decode)
    {
      ...
      case eImpl::Advanced: m_DecoderSimple.init(&m_OutBuffer); m_DecoderSimple.decode(&m_OutBuffer, m_PicRec4XX); break;
#if X_PMBB_HAS_JPEG_TURBO
      case eImpl::Turbo   : m_DecoderTurbo .decode(&m_OutBuffer, m_PicRec4XX); break;
#endif //X_PMBB_HAS_JPEG_TURBO
      default: return eAppRes::Error; break;
      }      
    }
    ...
    {
      if(m_ReorderRGB) { reorderRGB(); }
      xSeqPic::tResult WriteResult = !m_CvtClrSpc ? m_SeqRec->writeFrame(m_PicRec4XX) : m_SeqRec->writeFrame(m_PicRecRGB);
      if(!WriteResult) { xErrMsg::printError(fmt::format("ERROR --> InputFile write error ({}) {}", m_ReconFile, WriteResult.format())); return eAppRes::Error; }
    }
    ...
    { 
      if(m_CalkPSNR)
      {
        fmt::print("PSNR[dB]: Y={:.4f} Cb={:.4f} Cr={:.4f} ", m_FramePSNR_YUV[f][0], m_FramePSNR_YUV[f][1], m_FramePSNR_YUV[f][2]);
        if(m_CvtClrSpc) { fmt::print("R={:.4f} G={:.4f} B={:.4f} ", m_FramePSNR_RGB[f][0], m_FramePSNR_RGB[f][1], m_FramePSNR_RGB[f][2]); }
      }
      fmt::print("Size[B]={:d} ", m_FrameBits[f]>>3);
      fmt::print("\n");
    }
  }
...
eAppRes xAppJPEG::validateFrames()
{
  ...
  if(m_InvalidPelActn==eActn::STOP && !CheckOK) { xErrMsg::printError(fmt::format("ERROR --> InputFile contains invalid values ({})", m_InputFile)); return eAppRes::Error; }
...
flt64V4 xAppJPEG::calcPicPSNR(const xPicP* Tst, const xPicP* Ref, bool AvoidInfPSNR)
{
  ...
  flt64V4 PSNR = xMakeVec4(std::numeric_limits<flt64>::max());
...
flt64 xAppJPEG::calcCmpPSNR(const xPicP* Tst, const xPicP* Ref, eCmp CmpId, bool AvoidInfPSNR)
{
  ...
  uint64 SSD = xDistortion::CalcSSD(TstPtr, RefPtr, TstStride, RefStride, Width, Height, BitDepth);
...
void xAppJPEG::combineFrameStats()
{
  ...
  int64  OneFrameSize = m_SeqOrg->getOneFrameSize();
...
std::string xAppJPEG::formatResultsStdOut()
{
  ...
  if(m_PrintDebug)
  {
    ...
    switch(m_Implementation)
    {
    case eImpl::Simple  : Result += m_EncoderSimple.formatAndResetStats("  ", m_TimeStamp.getTicksPerMicroSec()) + "\n"; break;
    case eImpl::Advanced: Result += m_EncoderRDOQ  .formatAndResetStats("  ", m_TimeStamp.getTicksPerMicroSec()) + "\n"; break;
#if X_PMBB_HAS_JPEG_TURBO
    case eImpl::Turbo   : Result += m_EncoderTurbo .formatAndResetStats("  ") + "\n"; break;
#endif //X_PMBB_HAS_JPEG_TURBO
    default: break;
    }
    if(m_Decode)
    {
      ...
      switch(m_Implementation)
      {
      case eImpl::Simple  : Result += m_DecoderSimple.formatAndResetStats("  ", m_TimeStamp.getTicksPerMicroSec()) + "\n"; break;
      case eImpl::Advanced: Result += m_DecoderSimple.formatAndResetStats("  ", m_TimeStamp.getTicksPerMicroSec()) + "\n"; break;
#if X_PMBB_HAS_JPEG_TURBO
      case eImpl::Turbo   : Result += m_DecoderTurbo .formatAndResetStats("  ") + "\n"; break;
#endif //X_PMBB_HAS_JPEG_TURBO
      default: break;
      }
    }
