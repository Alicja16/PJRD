...
void xTransformFLT::FwdTransformDCT_8x8_MFL(int16* restrict Dst, const uint16* Src)
{
  ...
  for(int32 j=0; j < 8; j++) //horizontal transform
  {
    ...
    {
      flt32 Coeff = 0;
      for(int32 x=0; x < 8; x++) { Coeff += (flt32)Src[x] * tTC::c_TrM_DCT8x8_FWD_FLT[u][x]; }
      Tmp[u][j] = Coeff;
    }
    Src += 8;
  }
  ...
  for(int32 j=0; j < 8; j++) //vertical transform
  {
    ...
    {
      flt32 Coeff = 0;
      for(int32 y=0; y < 8; y++) { Coeff += Tmp[j][y] * tTC::c_TrM_DCT8x8_FWD_FLT[u][y]; }
      Dst[u*8] = (int16)(xRoundF32ToI32(Coeff*2));
    }
    Dst++;
  }
}
void xTransformFLT::InvTransformDCT_8x8_MFL(uint16* restrict Dst, const int16* Src)
{
  ...
  for(int32 j=0; j < 8; j++) //vertical transform
  {
    ...
    {
      flt32 Coeff = 0;
      for(int32 y=0; y < 8; y++) { Coeff += (flt32)Src[y*8] * tTC::c_TrM_DCT8x8_FWD_FLT[y][u]; }
      Tmp[u][j] = Coeff;
    }
    Src++;
  }
  ...
  for(int32 j=0; j < 8; j++) //horizontal transform
  {
    ...
    {
      flt32 Coeff = 0;
      for(int32 x=0; x < 8; x++) { Coeff += Tmp[j][x] * tTC::c_TrM_DCT8x8_FWD_FLT[x][u]; }
      Dst[u] = (uint16)xClipU16((int32)xRoundF32ToI32(Coeff/8));
    }
    Dst += 8;
  }
}
void xTransformFLT::FwdTransformDCT_8x8_BTF(int16* restrict Dst, const uint16* Src)
{
  flt32 E [4], O [4]; //level 1 even and odd
  flt32 EE[2], EO[2]; //level 2 even and odd
  flt32 Tmp[8][8];    //partial transformed
  ...
  for (int32 j=0; j<8; j++) //horizontal transform
  {
    ...
    for(int32 k=0; k < 4; k++)
    {
      E[k] = (flt32)Src[k] + (flt32)Src[7 - k];
      O[k] = (flt32)Src[k] - (flt32)Src[7 - k];
    }
    Src += 8;
    ...
    flt32 A = (EO[1] + EO[0]) * tTC::c_F0o541196100;
    Tmp[2][j] = A + (EO[0] *  tTC::c_F0o765366865);
    Tmp[6][j] = A + (EO[1] * -tTC::c_F1o847759065);

    //odd part
    flt32 B  = (O[0] + O[1] + O[2] + O[3]) * tTC::c_F1o175875602;
    flt32 B1 = (O[3] + O[0]) * -tTC::c_F0o899976223;
    flt32 B2 = (O[2] + O[1]) * -tTC::c_F2o562915447;
    flt32 B3 = (O[3] + O[1]) * -tTC::c_F1o961570560 + B;
    flt32 B4 = (O[2] + O[0]) * -tTC::c_F0o390180644 + B;

    Tmp[1][j] = O[0] * tTC::c_F1o501321110 + B1 + B4;
    Tmp[3][j] = O[1] * tTC::c_F3o072711026 + B2 + B3;
    Tmp[5][j] = O[2] * tTC::c_F2o053119869 + B2 + B4;
    Tmp[7][j] = O[3] * tTC::c_F0o298631336 + B1 + B3;    
  }
  ...
  {    
    ...
    //even part
    Dst[0*8 + j] = (int16)xRoundF32ToI32((EE[0] + EE[1]) * 2);
    Dst[4*8 + j] = (int16)xRoundF32ToI32((EE[0] - EE[1]) * 2);

    flt32 A = (EO[1] + EO[0]) * tTC::c_F0o541196100;
    Dst[2*8 + j] = (int16)xRoundF32ToI32((A + (EO[0] *  tTC::c_F0o765366865)) * 2);
    Dst[6*8 + j] = (int16)xRoundF32ToI32((A + (EO[1] * -tTC::c_F1o847759065)) * 2);
    ...
    Dst[1*8 + j] = (int16)xRoundF32ToI32((O[0] * tTC::c_F1o501321110 + B1 + B4) * 2);
    Dst[3*8 + j] = (int16)xRoundF32ToI32((O[1] * tTC::c_F3o072711026 + B2 + B3) * 2);
    Dst[5*8 + j] = (int16)xRoundF32ToI32((O[2] * tTC::c_F2o053119869 + B2 + B4) * 2);
    Dst[7*8 + j] = (int16)xRoundF32ToI32((O[3] * tTC::c_F0o298631336 + B1 + B3) * 2);
    //Dst++;
  }
}
void xTransformFLT::InvTransformDCT_8x8_BTF(uint16* restrict Dst, const int16* Src)
{
  flt32 E [4], O [4];
  flt32 EE[2], EO[2];
  flt32 Tmp[8][8];
  ...
  {
    //coeffs
    const flt32 Coeff0 = Src[0*8];
    const flt32 Coeff1 = Src[1*8];
    const flt32 Coeff2 = Src[2*8];
    const flt32 Coeff3 = Src[3*8];
    const flt32 Coeff4 = Src[4*8];
    const flt32 Coeff5 = Src[5*8];
    const flt32 Coeff6 = Src[6*8];
    const flt32 Coeff7 = Src[7*8];
    Src += 1;    

    //even part
    flt32 A  = (Coeff2 + Coeff6) * tTC::c_F0o541196100; //z1
    EE[0] = (Coeff0 + Coeff4); //tmp0
    EE[1] = (Coeff0 - Coeff4); //tmp3      
    EO[0] = A + (Coeff2 *  tTC::c_F0o765366865); //tmp3
    EO[1] = A + (Coeff6 * -tTC::c_F1o847759065); //tmp2
    ...
    //odd part
    flt32 B  = (Coeff1 + Coeff3 + Coeff5 + Coeff7) * tTC::c_F1o175875602; 
    flt32 B0 = (Coeff7 + Coeff1) * (-tTC::c_F0o899976223); //z1
    flt32 B1 = (Coeff5 + Coeff3) * (-tTC::c_F2o562915447); //z2
    flt32 B2 = (Coeff7 + Coeff3) * (-tTC::c_F1o961570560) + B;
    flt32 B3 = (Coeff5 + Coeff1) * (-tTC::c_F0o390180644) + B;

    O[3] = Coeff7 * tTC::c_F0o298631336 + B0 + B2;
    O[2] = Coeff5 * tTC::c_F2o053119869 + B1 + B3;
    O[1] = Coeff3 * tTC::c_F3o072711026 + B1 + B2;
    O[0] = Coeff1 * tTC::c_F1o501321110 + B0 + B3; //tmp3
  ...
  { 
    //coeffs
    const flt32 Coeff0 = Tmp[j][0];
    const flt32 Coeff1 = Tmp[j][1];
    const flt32 Coeff2 = Tmp[j][2];
    const flt32 Coeff3 = Tmp[j][3];
    const flt32 Coeff4 = Tmp[j][4];
    const flt32 Coeff5 = Tmp[j][5];
    const flt32 Coeff6 = Tmp[j][6];
    const flt32 Coeff7 = Tmp[j][7];

    //even part
    flt32 A  = (Coeff2 + Coeff6) * tTC::c_F0o541196100;
    EE[0] = (Coeff0 + Coeff4);
    EE[1] = (Coeff0 - Coeff4);
    EO[0] = A + (Coeff2 * ( tTC::c_F0o765366865));
    EO[1] = A + (Coeff6 * (-tTC::c_F1o847759065));
    ...
    //odd part
    flt32 B  = (Coeff1 + Coeff3 + Coeff5 + Coeff7) * tTC::c_F1o175875602; 
    flt32 B0 = (Coeff7 + Coeff1) * (-tTC::c_F0o899976223);
    flt32 B1 = (Coeff5 + Coeff3) * (-tTC::c_F2o562915447);
    flt32 B2 = (Coeff7 + Coeff3) * (-tTC::c_F1o961570560) + B;
    flt32 B3 = (Coeff5 + Coeff1) * (-tTC::c_F0o390180644) + B;
    ...
    O[2] = Coeff5 * tTC::c_F2o053119869 + B1 + B3;
    O[1] = Coeff3 * tTC::c_F3o072711026 + B1 + B2;
    O[0] = Coeff1 * tTC::c_F1o501321110 + B0 + B3;

    for(int32 k=0;k<4;k++)
    {
      Dst[k  ] = (uint16)xClipU16(xRoundF32ToI32((E[k] + O[k])/8));
      Dst[7-k] = (uint16)xClipU16(xRoundF32ToI32((E[k] - O[k])/8));
    }
    Dst+= 8;
  }
...
void xTransformSTD::FwdTransformDCT_8x8_M16(int16* restrict Dst, const uint16* Src)
{
  ...
  for(int32 j=0; j < 8; j++) //horizontal transform
  {
    ...
    {
      int32 Coeff = 0;
      for(int32 x=0; x < 8; x++) { Coeff += (int32)Src[x] * (int32)tTC::c_TrM_DCT8x8_FWD_16bit[u][x]; }
      int32 CoeffRes = (Coeff + tTC::c_FrwAdd1st_16bit) >> tTC::c_FrwShift1st_16bit;
      Tmp[u][j] = (int16)CoeffRes;
    }
    Src += 8;
  }
  ...
  for(int32 j=0; j < 8; j++) //vertical transform
  {
    ...
    {
      int32 Coeff = 0;
      for(int32 y=0; y < 8; y++) { Coeff += (int32)Tmp[j][y] * (int32)tTC::c_TrM_DCT8x8_FWD_16bit[u][y]; }
...
void xTransformSTD::InvTransformDCT_8x8_M16(uint16* restrict Dst, const int16* Src)
{
  ...
  for(int32 j=0; j < 8; j++) //vertical transform
  {
    ...
    {
      int32 Coeff = 0;
      for(int32 y=0; y < 8; y++) { Coeff += (int32)Src[y*8] * (int32)tTC::c_TrM_DCT8x8_INV_16bit[u][y]; }
  ...
  for(int32 j=0; j < 8; j++) //horizontal transform
  {
    ...
    {
      int32 Coeff = 0;
      for(int32 x=0; x < 8; x++) { Coeff += (int32)Tmp[j][x] * (int32)tTC::c_TrM_DCT8x8_INV_16bit[u][x]; }
...
{  
  constexpr int32 Shift1st  = tTC::c_CoeffPrec_BTF - PASS1_BITS;
  constexpr int32 Add1st    = 1<<(Shift1st-1); 
  constexpr int32 Shift2nd  = tTC::c_CoeffPrec_BTF + PASS1_BITS - 1;
  constexpr int32 Add2nd    = 1<<(Shift2nd-1); 
  constexpr int32 Add2ndX   = 1 << (PASS1_BITS - 2);
  ...
  for (int32 j=0; j<8; j++) //horizontal transform
  {
    ...
    for(int32 k=0; k < 4; k++)
    {
      E[k] = (int32)Src[k] + (int32)Src[7 - k];
      O[k] = (int32)Src[k] - (int32)Src[7 - k];
    }
    Src += 8;
...
void xTransformSTD::InvTransformDCT_8x8_BTF(uint16* restrict Dst, const int16* Src)
{
  constexpr int32 Shift1st  = tTC::c_CoeffPrec_BTF - PASS1_BITS;
  constexpr int32 Add1st    = 1<<(Shift1st-1); 
  constexpr int32 Shift2nd  = tTC::c_CoeffPrec_BTF + PASS1_BITS + 3;
  constexpr int32 Add2nd    = 1<<(Shift2nd-1); 
  int32 E [4], O [4];
  int32 EE[2], EO[2];
  int16 Tmp[8][8];
  ...
  { 
    ...
    for(int32 k=0;k<4;k++)
    {
      Dst[k  ] = (uint16)xClipU16(((E[k] + O[k] + Add2nd)>>Shift2nd));
      Dst[7-k] = (uint16)xClipU16(((E[k] - O[k] + Add2nd)>>Shift2nd));
    }
    Dst+= 8;
  }
}
...
void xTransformSSE::FwdTransformDCT_8x8_M16(int16* restrict Dst, const uint16* restrict Src)
{
...
#if LOAD_FROM_CONST
  const __m128i xTC0 = _mm_set1_epi16 (           tTC::c_TrM_DCT8x8_FWD_16bit[0][0]);
  const __m128i xTC1 = _mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[1][0]);
  const __m128i xTC2 = _mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[2][0]);
  const __m128i xTC3 = _mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[3][0]);
  const __m128i xTC4 = _mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[4][0]);
  const __m128i xTC5 = _mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[5][0]);
  const __m128i xTC6 = _mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[6][0]);
  const __m128i xTC7 = _mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[7][0]);
#else
  const __m128i xTC0 = _mm_setr_epi16(16384,  16384,  16384,  16384,  16384,  16384,  16384,  16384);
  const __m128i xTC1 = _mm_setr_epi16(22725,  19266,  12873,   4520,  -4520, -12873, -19266, -22725);
  const __m128i xTC2 = _mm_setr_epi16(21407,   8867,  -8867, -21407, -21407,  -8867,   8867,  21407);
  const __m128i xTC3 = _mm_setr_epi16(19266,  -4520, -22725, -12873,  12873,  22725,   4520, -19266);
  const __m128i xTC4 = _mm_setr_epi16(16384, -16384, -16384,  16384,  16384, -16384, -16384,  16384);
  const __m128i xTC5 = _mm_setr_epi16(12873, -22725,   4520,  19266, -19266,  -4520,  22725, -12873);
  const __m128i xTC6 = _mm_setr_epi16( 8867, -21407,  21407,  -8867,  -8867,  21407, -21407,   8867);
  const __m128i xTC7 = _mm_setr_epi16( 4520, -12873,  19266, -22725,  22725, -19266,  12873,  -4520);
#endif
  ...
  for(int32 j=0; j<8; j++)
  {
    ...
    __m128i Tr0V = _mm_madd_epi16(SrcV, xTC0);
    __m128i Tr1V = _mm_madd_epi16(SrcV, xTC1);
    __m128i Tr2V = _mm_madd_epi16(SrcV, xTC2);
    __m128i Tr3V = _mm_madd_epi16(SrcV, xTC3);
    __m128i Tr4V = _mm_madd_epi16(SrcV, xTC4);
    __m128i Tr5V = _mm_madd_epi16(SrcV, xTC5);
    __m128i Tr6V = _mm_madd_epi16(SrcV, xTC6);
    __m128i Tr7V = _mm_madd_epi16(SrcV, xTC7);
...
#if LOAD_FROM_CONST
  const __m128i xTC0 = _mm_loadu_si128((__m128i*) & tTC::c_TrM_DCT8x8_INV_16bit[0][0]);
  const __m128i xTC1 = _mm_loadu_si128((__m128i*) & tTC::c_TrM_DCT8x8_INV_16bit[1][0]);
  const __m128i xTC2 = _mm_loadu_si128((__m128i*) & tTC::c_TrM_DCT8x8_INV_16bit[2][0]);
  const __m128i xTC3 = _mm_loadu_si128((__m128i*) & tTC::c_TrM_DCT8x8_INV_16bit[3][0]);
  const __m128i xTC4 = _mm_loadu_si128((__m128i*) & tTC::c_TrM_DCT8x8_INV_16bit[4][0]);
  const __m128i xTC5 = _mm_loadu_si128((__m128i*) & tTC::c_TrM_DCT8x8_INV_16bit[5][0]);
  const __m128i xTC6 = _mm_loadu_si128((__m128i*) & tTC::c_TrM_DCT8x8_INV_16bit[6][0]);
  const __m128i xTC7 = _mm_loadu_si128((__m128i*) & tTC::c_TrM_DCT8x8_INV_16bit[7][0]);
#else
  const __m128i xTC0 = _mm_setr_epi16(16384, 22725, 21407, 19266, 16384, 12873,  8867,  4520);
  const __m128i xTC1 = _mm_setr_epi16(16384, 19266,  8867, -4520,-16384,-22725,-21407,-12873);
  const __m128i xTC2 = _mm_setr_epi16(16384, 12873, -8867,-22725,-16384,  4520, 21407, 19266);
  const __m128i xTC3 = _mm_setr_epi16(16384,  4520,-21407,-12873, 16384, 19266, -8867,-22725);
  const __m128i xTC4 = _mm_setr_epi16(16384, -4520,-21407, 12873, 16384,-19266, -8867, 22725);
  const __m128i xTC5 = _mm_setr_epi16(16384,-12873, -8867, 22725,-16384, -4520, 21407,-19266);
  const __m128i xTC6 = _mm_setr_epi16(16384,-19266,  8867,  4520,-16384, 22725,-21407, 12873);
  const __m128i xTC7 = _mm_setr_epi16(16384,-22725, 21407,-19266, 16384,-12873,  8867, -4520);
#endif
...
#if LOAD_FROM_CONST
  const __m256i xTC0_V = _mm256_set1_epi16     (                           tTC::c_TrM_DCT8x8_FWD_16bit[0][0] );
  const __m256i xTC1_V = _mm256_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[1][0]));
  const __m256i xTC2_V = _mm256_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[2][0]));
  const __m256i xTC3_V = _mm256_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[3][0]));
  const __m256i xTC4_V = _mm256_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[4][0]));
  const __m256i xTC5_V = _mm256_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[5][0]));
  const __m256i xTC6_V = _mm256_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[6][0]));
  const __m256i xTC7_V = _mm256_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[7][0]));
#else
...
#if LOAD_FROM_CONST
  const __m256i xTC0_V = _mm256_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_INV_16bit[0][0]));
  const __m256i xTC1_V = _mm256_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_INV_16bit[1][0]));
  const __m256i xTC2_V = _mm256_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_INV_16bit[2][0]));
  const __m256i xTC3_V = _mm256_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_INV_16bit[3][0]));
  const __m256i xTC4_V = _mm256_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_INV_16bit[4][0]));
  const __m256i xTC5_V = _mm256_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_INV_16bit[5][0]));
  const __m256i xTC6_V = _mm256_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_INV_16bit[6][0]));
  const __m256i xTC7_V = _mm256_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_INV_16bit[7][0]));
#else
...
#if LOAD_FROM_CONST
  const __m512i xTC0_V = _mm512_set1_epi16(                                tTC::c_TrM_DCT8x8_FWD_16bit[0][0] );
  const __m512i xTC1_V = _mm512_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[1][0]));
  const __m512i xTC2_V = _mm512_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[2][0]));
  const __m512i xTC3_V = _mm512_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[3][0]));
  const __m512i xTC4_V = _mm512_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[4][0]));
  const __m512i xTC5_V = _mm512_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[5][0]));
  const __m512i xTC6_V = _mm512_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[6][0]));
  const __m512i xTC7_V = _mm512_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_FWD_16bit[7][0]));
#else
...
#if LOAD_FROM_CONST
  const __m512i xTC0_V = _mm512_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_INV_16bit[0][0]));
  const __m512i xTC1_V = _mm512_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_INV_16bit[1][0]));
  const __m512i xTC2_V = _mm512_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_INV_16bit[2][0]));
  const __m512i xTC3_V = _mm512_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_INV_16bit[3][0]));
  const __m512i xTC4_V = _mm512_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_INV_16bit[4][0]));
  const __m512i xTC5_V = _mm512_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_INV_16bit[5][0]));
  const __m512i xTC6_V = _mm512_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_INV_16bit[6][0]));
  const __m512i xTC7_V = _mm512_broadcast_i32x4(_mm_loadu_si128((__m128i*)&tTC::c_TrM_DCT8x8_INV_16bit[7][0]));
#else
...
void xTransformNEON::FwdTransformDCT_8x8_M16(int16* restrict Dst, const uint16* restrict Src)
{

}
void xTransformNEON::InvTransformDCT_8x8_M16(uint16* restrict Dst, const int16* restrict Src)
{
