...
template<class T, std::size_t N> std::array<T, N> xMakeArray(T V)
{
  std::array<T, N> tempArray{};
  for(size_t i=0; i<N; i++){ tempArray[i] = V; }
  return tempArray;
}
...
{
protected:
  uint16 m_QuantFltR[64];

public:
  const uint16* getQuantFlt  () const { return m_QuantFltR  ; };
  const uint16* getQuantCoeff() const { return m_QuantCoeffR; };
  const uint16* getReciprocal() const { return m_ReciprocalR; };
  const uint16* getCorrection() const { return m_CorrectionR; };
  const uint16* getScale     () const { return m_ScaleR     ; };
  const uint16* getShift     () const { return m_ShiftR     ; };

  void initQuantFlt()
  {
    for(int32 i = 0; i < 64; i++) { m_QuantFltR[i] = m_QuantCoeffR[i] << xJPEG_Constants::c_FwdTransformHeadroom; }
  }
...
void testQuantSTD(bool Verbose)
{
  {
    const std::array<int16, 65536> TmpSequence = [] { std::array<int16, 65536> R{}; for(int32 i = 0; i < 65536; i++) { R[i] = (int16)(i - 32768); } return R; }();
    ...
    for(uint16 Divisor = 1; Divisor < 256; Divisor++)
    {
      ...
      CHECK(xTestUtils::isSameBuffer(RefQ.data(), TstQ.data(), 65536, true));
  ...
  if(Verbose){
    for(uint16 D = 1; D < 256; D++)
    {
      ...
      uint16 Shift;
      xQuantTest::ComputeReciprocal(Divisor, Reciprocal, Correction, Scale, Shift);
      fmt::print("Divisor={:3d} Reciprocal={} Correction={} Scale={} Shift={}\n", Divisor, Reciprocal, Correction, Scale, Shift);
    }
    for(uint16 D = 1; D < 256; D++)
    {
      uint16 Divisor = D << xJPEG_Constants::c_FwdTransformHeadroom;
      uint16 Reciprocal;
      uint16 Correction;
      uint16 Scale;
      uint16 Shift;
...
void testQuantSIMD(std::function <void(int16*, const int16*, const uint16*, const uint16*, const uint16*)>QuantScale, std::function <void(int16*, const int16*, const uint16*)>InvScale, bool UseScale)
{
  xQuantTest Quantizer;
  std::array<int16, BA> Src;
  std::array<int16, BA> RefQ;
  ...
  for(int32 Quality = 100; Quality >= 0; Quality--)
  {
    ...
    for(int32 r = 0; r < 1024; r++)
    {
      ...
      if(UseScale){ QuantScale(TstQ.data(), Src.data(), Quantizer.getCorrection(), Quantizer.getReciprocal(), Quantizer.getScale());}
      else        { QuantScale(TstQ.data(), Src.data(), Quantizer.getCorrection(), Quantizer.getReciprocal(), Quantizer.getShift());}
      CHECK(xTestUtils::isSameBuffer(TstQ.data(), RefQ.data(), BA, true));
...
TEST_CASE("xQuantSSE")
{
  testQuantSIMD(xQuantSSE::QuantScale, xQuantSSE::InvScale, true);
}
#endif
...
TEST_CASE("xQuantAVX")
{
  testQuantSIMD(xQuantAVX::QuantScale, xQuantAVX::InvScale, true);
}
#endif
...
{
  testQuantSIMD(xQuantAVX512::QuantScale, xQuantAVX512::InvScale, true);
}
#endif

#if X_SIMD_CAN_USE_NEON
TEST_CASE("xQuantNEON")
{
  testQuantSIMD(xQuantNEON::QuantScale, xQuantNEON::InvScale, false);
}
#endif
